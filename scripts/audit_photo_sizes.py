...
def print_report(large_images, threshold_kb):
    ...
    if not large_images:
        print(f"\n[OK] No images found larger than {threshold_kb}KB")
        return
...
def optimize_images(root_dir, large_images, quality=85):
    ...
    print(f"\n[OPTIMIZE] Optimizing images (quality={quality})")
    print(f"[BACKUP] Backing up to: {backup_dir.relative_to(root_dir)}")
    print(f"{'=' * 60}\n")
    ...
    for img_info in large_images:
        ...
        if file_path.suffix.lower() == '.svg':
            print(f"[SKIP] Skipping SVG: {relative_path}")
            continue
        ...
        try:
            ...
            if saved > 0:
                ...
                print(f"[SKIP] {relative_path} (already optimized)")

        except Exception as e:
            print(f"[ERROR] Error processing {relative_path}: {e}")
...
def main():
    ...
    print(f"[SCAN] Scanning for images > {args.threshold}KB...")
